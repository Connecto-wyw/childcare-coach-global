...
export function getSystemPrompt(opts: PromptOpts = {}) {
  ...
  const followupRule = followupEnabled
    ? '요약 뒤에 바로 이어서, 이번 주제와 직접 연결된 다음 한 가지 행동이나 정보를 제안하고, 그 제안에 맞는 맞춤형 질문으로 끝내세요. 예: "추가 자료를 알려드릴까요?", "실행 방법을 정리해드릴까요?", "다른 사례도 설명해드릴까요?"와 같이 상황에 어울리게 변형하세요.'
    : '후속 제안은 하지 마세요.';
  ...
  const endRule =
    '출력 종료 규칙: 맨 마지막 줄에는 반드시 "요약: ..." 한 줄 요약을 작성하고, 그 다음 줄에 후속 제안을 넣은 뒤 [END]로 종료하세요. [END] 앞뒤 공백 금지.';
