// src/app/admin/team/[teamId]/activities/new/page.tsx
...
async function createAction(teamId: string, formData: FormData) {
  'use server'
...
export default async function NewActivityPage({ params }: { params: { teamId: string } }) {
  ...
  return (
    <main className="min-h-screen bg-[#333333] text-[#eae3de] font-sans">
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        <form action={createAction.bind(null, teamId)} className="mt-6 rounded-lg border border-gray-700 bg-[#222] p-6">
          ...
          <div className="mt-6 flex justify-end gap-2">
            <Link className="rounded bg-gray-600 px-4 py-2 text-white font-semibold hover:opacity-90" href={`/admin/team/${teamId}/activities`}>
              Cancel
            </Link>
        ...
        <p className="mt-4 text-xs text-gray-400">
          현재 DB 스키마 기준: title/description/starts_at/ends_at만 저장함.  
          (이미지/정렬/is_active는 컬럼 추가 후 2단계에서 붙이면 됨)
        </p>
      </div>
    </main>
  )
