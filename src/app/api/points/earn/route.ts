...
export async function POST(req: NextRequest) {
  try {
    ...
    const currentPoints = Number((profileRes.data as any)?.points ?? 0)
    const monthlyEarned = (monthlyRes.data ?? []).reduce((s: number, r: any) => s + r.amount, 0)

    // 월 무료 한도 초과 체크
    if (monthlyEarned >= MAX_FREE_POINTS) {
      return NextResponse.json({ ok: false, error: 'limit_reached', monthlyEarned }, { status: 200 })
    }
    ...
    if (ONE_TIME_REASONS.includes(reason)) {
      const { count } = await (supabase as any)
        ...
        .eq('reason', reason)
      if ((count ?? 0) > 0) {
        return NextResponse.json({ ok: false, error: 'already_earned', monthlyEarned }, { status: 200 })
      }
    }
    ...
    if (dailyLimit !== undefined) {
      ...
      if (remaining <= 0) {
        return NextResponse.json({ ok: false, error: 'daily_limit_reached', monthlyEarned }, { status: 200 })
      }
      pointsAdded = Math.min(amount, remaining)
    }

    // 월 3000P 한도 초과 방지
    pointsAdded = Math.min(pointsAdded, MAX_FREE_POINTS - monthlyEarned)
    ...
    // profiles.points 업데이트 (누적 잔액)
    const newTotal = currentPoints + pointsAdded
    await supabase.from('profiles').upsert({ id: user.id, points: newTotal })

    return NextResponse.json({ ok: true, pointsAdded, newTotal, monthlyEarned: monthlyEarned + pointsAdded })
  } catch (e) {
